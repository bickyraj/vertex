...
var KTDatatableJsonRemoteDemo = function () {
		...
		dataTable = $('.kt-datatable').KTDatatable({
			...
			columns: [
				...
				// },
				{
					...
					template: function(item) {
						return '\
						<a href="'+url+'/admin/trip-faqs/'+item.id+'/list" class="btn btn-clean btn-sm" title="Edit details">\
							FAQ\'s\
						</a>\
	...
	$(document).on('change', '#publishSwitch', function(e) {
		var id = $(this).data('id');
		var action_url = url + '/admin/trip-faqs/publish/' + id; 
		$.ajax({
			url: action_url,
			type: "GET",
			dataType: "json",
	...
	$(document).on('click', '.kt_sweetalert_delete_page', function(event) {
		...
		swal.fire({
		    title: 'Are you sure you want to delete all the faqs of this trip?',
		    ...
		    if (result.value) {
		    	...
		    	var action_url = url + '/admin/trips/faqs/delete/' + id; 
		    	$.ajax({
		    		url: action_url,
		    		type: "DELETE",
		    		dataType: "json",
